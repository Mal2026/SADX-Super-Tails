...
void NjSetTexture_Flicky() {
	...
	if (isTailsCurChar() && SuperSonicFlag) {

		if (isDCCharUsed)
			njSetTexture(&FlickyDC_TEXLIST);
		else
			njSetTexture(&FlickyDX_TEXLIST);
...
void FlickyAction_Main_r(ObjectMaster* obj) {
	...
	if (EntityData1Ptrs[0]->CharID != Characters_Tails) //restore original function for Amy bird.
	{
